...
For session re-entries, the summary classifies every reusable prefix byte
into a joint residency table: hit on both prefill and decode, decode only,
prefill only, or neither. First turns and novel suffixes are excluded. The
decode-miss cells are bytes that must cross the hand-off link; the
prefill-miss cells are positions that must be recomputed. Those prefiller
misses are also split by provenance: KV materialized by the parent turn's
prefill, KV generated by its decoder, or inherited context from before a
stationary mid-trace start. Under same-rank session affinity, the first exposes
cache churn while the second is the structural cost of not writing output KV
back to the prefiller. Per-decode-worker rows preserve DPA-rank skew instead of
hiding it in the aggregate.
